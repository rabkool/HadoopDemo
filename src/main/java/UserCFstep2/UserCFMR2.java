 ...
 * 输入 : step1的输出
 * 缓存 : step1的输出
 * 输出 : 用户id(行)_用户id(列)_相似度
 * 
 * @author 90595
 *
 */
public class UserCFMR2 {
	...
	public int run() {
		...
		try {
			...
			// 創建一個job實列
			Job job = Job.getInstance(conf, "UserCFstep2");

			job.addCacheFile(new URI(cache + "#UserScore1"));

			// 設置job的主類
			job.setJarByClass(UserCFMR2.class);
			// 設置job的mapper類的reducer類
			job.setMapperClass(UserCFMapper2.class);
			job.setReducerClass(UserCFReducer2.class);
