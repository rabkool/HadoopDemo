 ...
 * 输入 : step4的输出
 * 缓存 : step1的输出
 * 输出 : y用户id(行)_物品id(列)_分值 
 * 最终的推荐列表
 * 
 * @author 90595
 *
 */
public class ItemCFMR5 {
	// 输入文件的相对路径
	...
	public int run() {
		...
		try {
			...
			// 創建一個job實列
			Job job = Job.getInstance(conf, "ItemCFstep5");
			//添加缓存文件
			job.addCacheFile(new URI(cache + "#itemScore3"));

			// 設置job的主類
			job.setJarByClass(ItemCFMR5.class);
			// 設置job的mapper類的reducer類
			job.setMapperClass(ItemCFMapper5.class);
			job.setReducerClass(ItemCFReducer5.class);
